...
type AbwesenheitModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: AbwesenheitData) => void;
  initialData?: Partial<AbwesenheitData>;
  isLoading?: boolean;
};

export default function AbwesenheitModal({
  isOpen,
  onClose,
  onSubmit,
  initialData,
  isLoading = false,
}: AbwesenheitModalProps) {
  const today = new Date().toISOString().split('T')[0];
  
  const [formData, setFormData] = useState<AbwesenheitData>({
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof AbwesenheitData, string>>>({});
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (errors[name as keyof AbwesenheitData]) {
      setErrors(prev => ({ ...prev, [name]: undefined }));
    }
  };

  const validateForm = (): boolean => {
    const newErrors: Partial<Record<keyof AbwesenheitData, string>> = {};
    ...
    if (!formData.reason || formData.reason.trim() === '') {
      newErrors.reason = 'Grund für die Abwesenheit ist erforderlich';
    }
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-start space-x-2">
          <div>
            <input
              ...
              className="h-4 w-4 text-blue-500 rounded border-gray-300 focus:ring-blue-500 mt-1"
            />
          </div>
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-500">
              Aktivieren, wenn es sich um einen halben Tag Abwesenheit handelt
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 text-sm text-gray-500 bg-gray-50 p-3 rounded-md">
          <p>Hinweis: Abwesenheiten werden automatisch in der Stundenerfassung berücksichtigt.</p>
        </div>
      </form>
    </Modal>
  );
