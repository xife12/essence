...
type KrankheitModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: KrankheitData) => void;
  initialData?: Partial<KrankheitData>;
  isLoading?: boolean;
};

export default function KrankheitModal({
  isOpen,
  onClose,
  onSubmit,
  initialData,
  ...
  const [formData, setFormData] = useState<KrankheitData>({
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof KrankheitData, string>>>({});
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (errors[name as keyof KrankheitData]) {
      setErrors(prev => ({ ...prev, [name]: undefined }));
    }
  };

  const validateForm = (): boolean => {
    const newErrors: Partial<Record<keyof KrankheitData, string>> = {};
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-start space-x-2">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-500">
              Aktivieren, wenn es sich um einen halben Krankheitstag handelt
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 text-sm text-gray-500 bg-gray-50 p-3 rounded-md">
          <p>Hinweis: Krankheitstage werden automatisch in der Stundenerfassung berücksichtigt.</p>
        </div>
      </form>
    </Modal>
  );
