-- Migration 002: Standard-Daten und Stored Procedures für Vertragsarten V2
-- Fügt die 9 Standard-Kategorien und wichtige Stored Procedures hinzu

-- ============================================================================
-- PHASE 1: STANDARD MODUL-KATEGORIEN EINFÜGEN
-- ============================================================================
...
-- ============================================================================
-- PHASE 2: STORED PROCEDURES FÜR BULK-OPERATIONEN
-- ============================================================================
...
-- ============================================================================
-- PHASE 3: VIEWS FÜR RÜCKWÄRTSKOMPATIBILITÄT
-- ============================================================================
...
-- ============================================================================
-- PHASE 5: RLS POLICIES ERSTELLEN
-- ============================================================================
...
CREATE POLICY "contracts_insert_policy" ON public.contracts
    ...
    WITH CHECK (
        CASE 
            ...
            WHEN auth.jwt() ->> 'role' = 'studioleiter' THEN true
            ELSE false
        END
    );
...
-- Policies für Module (ähnlich)
CREATE POLICY "modules_select_policy" ON public.contract_modules
    FOR SELECT TO authenticated 
    USING (true);

CREATE POLICY "modules_modify_policy" ON public.contract_modules
    FOR ALL TO authenticated 
    USING (auth.jwt() ->> 'role' IN ('admin', 'studioleiter'));

-- Policies für Kategorien (alle können lesen)
CREATE POLICY "categories_select_policy" ON public.module_categories
    FOR SELECT TO authenticated 
    USING (true);

CREATE POLICY "categories_modify_policy" ON public.module_categories
    FOR ALL TO authenticated 
    USING (auth.jwt() ->> 'role' = 'admin');

-- ============================================================================
-- PHASE 6: SAMPLE DATA (Optional - für Entwicklung)
-- ============================================================================

-- Beispiel-Module einfügen (nur für Development)
DO $$
BEGIN
    IF current_setting('app.environment', true) = 'development' THEN
        ...
        SELECT 
            'Gruppenkurse',
            'Zugang zu allen Gruppenfitnesskursen',
            10.00,
            cat.id,
            'Dumbbell'
        ...
        RAISE NOTICE '🧩 Beispiel-Module für Development eingefügt';
    END IF;
END $$;
...
DO $$
BEGIN
    RAISE NOTICE '✅ Standard-Daten und Stored Procedures erfolgreich erstellt!';
    RAISE NOTICE '📂 Eingefügt: 9 Modul-Kategorien';
    RAISE NOTICE '⚙️ Erstellt: 5 Stored Procedures, 3 Views, RLS Policies';
    RAISE NOTICE '🔄 Nächster Schritt: TypeScript-Typen und API-Layer erstellen';
END $$;