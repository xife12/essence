...
export default function Dashboard() {
  ...
  const [contractDistribution, setContractDistribution] = useState<ContractDistribution[]>([]);
  const [terminationWarnings, setTerminationWarnings] = useState<TerminationWarning[]>([]);
  const [loading, setLoading] = useState(true);
  
  useEffect(() => {
    // Daten laden, wenn der Benutzer authentifiziert ist
    if (user) {
      const loadDashboardData = async () => {
        try {
          setLoading(true);
          
          // Diese Aufrufe würden normalerweise echte Daten aus Supabase laden
          // Hier verwenden wir Mock-Daten, bis die API-Verbindung hergestellt ist
          
          // Beispiel für eine tatsächliche API-Anfrage:
          // const { data: campaignsData } = await api.campaigns.getAll();
      ...
      <div className="grid grid-cols-12 gap-6 mb-6">
        ...
        <div className="col-span-12 sm:col-span-6 md:col-span-4 lg:col-span-3">
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="mb-2">
              <span className="text-2xl font-bold">{terminationWarnings.length}</span>
              <span className="text-sm text-gray-500 ml-2">in 90 Tagen</span>
            </div>
            <ul className="space-y-2">
              {terminationWarnings.map((warning) => (
